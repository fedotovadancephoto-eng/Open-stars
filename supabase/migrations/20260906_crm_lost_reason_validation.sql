-- Return a stable business error when a lost lead has no reason.
...
begin
  ...
  if coalesce(p_is_lost,false)
     and (p_lost_reason is null or p_lost_reason not in ('no_answer','not_responding','rescheduled','refusal','other_school','unqualified')) then
...
revoke all on function public.crm_update_lead(uuid,text,timestamptz,timestamptz,text,boolean,text,uuid)
  from public, anon, authenticated;
grant execute on function public.crm_update_lead(uuid,text,timestamptz,timestamptz,text,boolean,text,uuid)
  to authenticated;
