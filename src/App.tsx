  ...
  return (
    <div className="grid min-h-screen place-items-center bg-[#FAF9F5]">
      <div className="text-center">
        <div className="text-xl font-semibold tracking-[-0.03em] text-[#171717]">
          OPEN STARS
        </div>
        <div className="mt-2 text-xs uppercase tracking-[0.18em] text-black/35">
          {text}
        </div>
      </div>
    </div>
...
function App() {
  ...
  if (dataStatus === "idle" || dataStatus === "loading") {
    return <LoadingScreen text="Загружаем кабинет" />;
  }
      ...
      <div className="grid min-h-screen place-items-center bg-[#FAF9F5] px-6">
        <div className="w-full max-w-md rounded-[28px] border border-black/[0.06] bg-white p-6 text-center shadow-sm">
          ...
          <button
            ...
            className="mt-5 rounded-[16px] bg-[#171717] px-5 py-3 text-sm font-semibold text-white"
          >
            Повторить
          </button>
        </div>
      ...
      <div className="min-w-0 flex-1">
        ...
        <main className="mx-auto w-full max-w-7xl px-5 py-7 sm:px-6 lg:px-8">
          <section className="mb-6">
            <div className="inline-flex items-center rounded-full bg-gradient-to-r from-[#D96A24] to-[#E98A34] px-5 py-2.5 text-sm font-medium text-white shadow-sm">
              {ROLE_PORTAL_LABELS[DEMO_USER_ROLE]}
            </div>

            <h1 className="mt-5 text-3xl font-semibold tracking-[-0.035em] text-[#171717] sm:text-4xl">
              С возвращением!
            </h1>

            <p className="mt-2 text-[15px] text-black/50 sm:text-base">
              Всё важное об {nameInAboutCase(childFirstName)} в OPEN STARS — на этой неделе.
            </p>
          </section>
          ...
          <section className="mt-6">
            <OverviewCards onTabSelect={handleTabSelect} />
          </section>
          ...
          <section className="mt-7">
            ...
            <nav className="grid grid-cols-2 gap-2 rounded-[24px] border border-black/[0.055] bg-white p-2 shadow-[0_8px_28px_rgba(0,0,0,0.035)] sm:grid-cols-3 lg:grid-cols-5">
              {developmentTabs.map((tab) => {
                ...
                return (
                  ...
                  >
                    <span
                      className={`grid h-10 w-10 shrink-0 place-items-center rounded-[14px] transition-all ${
                        isActive ? "bg-white/[0.10]" : tab.iconBox
                      }`}
                    >
                      <Icon
                        size={20}
                        strokeWidth={2.2}
                        className={isActive ? "text-[#E8752A]" : tab.iconColor}
                      />
                    </span>
          ...
          <section className="mt-5">
            ...
            <nav className="grid grid-cols-2 gap-2 rounded-[24px] border border-black/[0.055] bg-white p-2 shadow-[0_8px_28px_rgba(0,0,0,0.035)] lg:grid-cols-4">
              {parentTabs.map((tab) => {
                const Icon = tab.icon;
                const isActive = activeTab === tab.id;
