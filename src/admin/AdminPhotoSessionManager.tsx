      ...
      <div className="max-h-[96vh] w-full max-w-5xl overflow-y-auto rounded-t-[28px] bg-[#FAF9F5] p-5 shadow-2xl sm:rounded-[28px] sm:p-7" onClick={(e) => e.stopPropagation()}>
        ...
        <section className="mt-6 rounded-[24px] border border-black/[0.06] bg-white p-5 sm:p-6">
          <div className="grid gap-3 sm:grid-cols-2 lg:grid-cols-4">
            ...
            <label className="text-xs font-semibold text-black/55">Группа<select value={groupName} onChange={(e) => setGroupName(e.target.value)} className={inputClass}>{groups.map((v) => <option key={v} value={v}>{groupLabel(v, branch, lessonDay, lessonTime)}</option>)}</select></label>
          ...
          <div className="mt-4 rounded-[16px] bg-[#FAF9F5] px-4 py-3 text-sm text-black/50"><span className="font-semibold text-[#171717]">Получатели:</span> {branch} · {lessonDay} · {lessonTime} · {groupLabel(groupName, branch, lessonDay, lessonTime)}</div>
