...
export function FinanceRegister({kind, lockedBranch="",onEditExpense,onPayroll,drilldown}:{kind:"payments"|"expenses";lockedBranch?:string;onEditExpense?:(row:FinanceRow)=>void;onPayroll?:(row:FinanceRow)=>void;drilldown?:ExpenseDrilldown|null}) {
 ...
 return <section ref={root} className="mt-5 rounded-3xl border border-black/5 bg-white p-5">
  ...
  {kind==="expenses" && row.editable && onEditExpense && <button type="button" className={`${control} mt-3`} onClick={()=>onEditExpense(row)}>Исправить расход</button>}
  {kind==="expenses" && row.source==="teacher_payroll" && onPayroll && <button type="button" className={`${control} mt-3`} onClick={()=>onPayroll(row)}>Исправить зарплату</button>}
