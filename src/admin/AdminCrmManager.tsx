...
export function AdminCrmManager() {
  ...
  async function refresh() {
    ...
    const branch = role === "admin" ? staffBranch : branchFilter;
    const [nextLeads, nextTasks] = await Promise.all([fetchCrmLeads(branch), fetchCrmTasks()]);
    setLeads(nextLeads); setTasks(nextTasks);
  }
  ...
  async function saveNewLead() {
    ...
    try {
      await createCrmLead({
        ...
        childName, parentName, parentPhone, source, sourceNote,
        trialAt: toIso(trialAt), nextContactAt: toIso(nextContactAt), comment: createComment,
      });
      await refresh();
      setChildName(""); setParentName(""); setParentPhone(""); setSource("Instagram"); setSourceNote(""); setTrialAt(""); setNextContactAt(localDateTime()); setCreateComment(""); setShowCreate(false);
  ...
  return <div className="fixed inset-0 z-[82] overflow-y-auto bg-[#F7F5EF]">
    <div className="mx-auto max-w-5xl px-4 pb-24 pt-[calc(env(safe-area-inset-top,0px)+1rem)] sm:px-6">
      ...
      {role === "marketer" ? <>
        ...
        <section className="mt-4 rounded-[22px] bg-white p-4"><h2 className="font-semibold">Источники</h2><div className="mt-3 space-y-2">{marketingRows.map(row=><div key={`${row.branch}-${row.source}`} className="flex justify-between rounded-[14px] bg-[#F7F5EF] p-3"><div><b>{row.source}</b><p className="text-xs text-black/40">{row.branch}</p></div><p className="text-right text-xs">{row.leads} лидов · {row.trials} пробных<br/>{row.students} учеников</p></div>)}</div><p className="mt-4 text-xs leading-5 text-black/35">Телефоны родителей и карточки детей маркетологу не показываются.</p></section>
      </> : <>
