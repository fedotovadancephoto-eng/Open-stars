...
import { CalendarDays, CheckCircle2, Clock3, LoaderCircle, Plus, Trash2, UsersRound, X } from "lucide-react";
...
export function AdminScheduleManager() {
  ...
  async function publish() {
    ...
    if (lessons.some((lesson) => !lesson.subject.trim())) {
      setError("Заполните название всех трёх уроков.");
      return;
    }
    setSaving(true);
    setError("");
    ...
    try {
      ...
      if (maxStudents === 0) {
        setSuccess(`Расписание сохранено на ${weeks} нед. Сейчас в этом потоке нет учеников; новые карточки подхватят занятия автоматически.`);
      } else {
        setSuccess(`Готово: расписание опубликовано на ${weeks} нед. для ${maxStudents} ученик(ов).`);
      }
      await refresh();
    } catch (err) {
  ...
  return (
    <>
      <button
        type="button"
        onClick={openManager}
        className="fixed bottom-[12.7rem] right-4 z-40 flex items-center gap-2 rounded-full bg-[#171717] px-4 py-3 text-sm font-semibold text-white shadow-[0_10px_30px_rgba(0,0,0,0.18)] sm:bottom-[13.4rem] sm:right-6"
      >
        <CalendarDays size={17} /> Расписание
      </button>

      {open && (
        <div className="fixed inset-0 z-[76] flex items-end justify-center bg-black/30 p-0 backdrop-blur-[2px] sm:items-center sm:p-5" onClick={() => !saving && setOpen(false)}>
          <div className="max-h-[95vh] w-full max-w-5xl overflow-y-auto rounded-t-[28px] bg-[#FAF9F5] p-5 shadow-2xl sm:rounded-[28px] sm:p-7" onClick={(event) => event.stopPropagation()}>
            <div className="flex items-start justify-between gap-4">
              <div>
                ...
                <h2 className="mt-1 text-2xl font-semibold tracking-[-0.03em]">Расписание групп</h2>
                <p className="mt-2 max-w-2xl text-sm leading-6 text-black/45">Выберите поток и три урока. Время уроков и перемен система рассчитывает сама.</p>
              </div>
              <button type="button" onClick={() => setOpen(false)} disabled={saving} className="grid h-10 w-10 shrink-0 place-items-center rounded-full bg-white text-black/55 shadow-sm disabled:opacity-50"><X size={20} /></button>
            </div>
              ...
              <div className="mt-6 rounded-[24px] border border-black/[0.06] bg-white p-5 sm:p-6">
                <div className="grid gap-3 sm:grid-cols-2 lg:grid-cols-5">
                  <label className="text-xs font-semibold text-black/55">Филиал
                    <select className={inputClass} value={branch} disabled={adminLocked} onChange={(event) => setBranch(event.target.value as ScheduleBranch)}>{branches.map((item) => <option key={item}>{item}</option>)}</select>
                  </label>
                  <label className="text-xs font-semibold text-black/55">Группа
                    <select className={inputClass} value={groupName} onChange={(event) => setGroupName(event.target.value as ScheduleGroup)}>{groups.map((item) => <option key={item}>{item}</option>)}</select>
                  </label>
                  <label className="text-xs font-semibold text-black/55">Поток
                    <select className={inputClass} value={stream} onChange={(event) => setStream(event.target.value as ScheduleStream)}>{streams.map((item) => <option key={item}>{item}</option>)}</select>
                  </label>
                  <label className="text-xs font-semibold text-black/55">Первое занятие
                    <input type="date" className={inputClass} value={firstDate} onChange={(event) => setFirstDate(event.target.value)} />
                  </label>
                  <label className="text-xs font-semibold text-black/55">Повторять
                    <select className={inputClass} value={weeks} onChange={(event) => setWeeks(Number(event.target.value))}>{[1,2,3,4,5,6,7,8].map((item) => <option key={item} value={item}>{item} нед.</option>)}</select>
                  </label>
                </div>
                ...
                <p className="mt-3 text-xs leading-5 text-black/35">Для 11:00 уроки по 30 минут и перемены по 5 минут. Для 13:00 и 16:00 — уроки по 45 минут и перемены по 10 минут.</p>
                <button type="button" onClick={publish} disabled={saving} className="mt-5 flex w-full items-center justify-center gap-2 rounded-[15px] bg-[#D96A24] px-5 py-3.5 text-sm font-semibold text-white disabled:opacity-50 sm:w-auto">
                  {saving ? <LoaderCircle className="animate-spin" size={18} /> : <Plus size={18} />}{saving ? "Публикуем..." : "Опубликовать расписание"}
                </button>
              </div>
            )}
                  ...
                  {schedules.map((schedule) => (
                    <div key={schedule.id} className="rounded-[20px] border border-black/[0.055] bg-white p-4 sm:p-5">
                      <div className="flex flex-col gap-3 sm:flex-row sm:items-start sm:justify-between">
                        ...
                        {canManage && <button type="button" disabled={busyId === schedule.id} onClick={() => remove(schedule)} className="flex shrink-0 items-center gap-1.5 self-start rounded-[11px] bg-red-50 px-3 py-2 text-xs font-semibold text-red-600 disabled:opacity-50">{busyId === schedule.id ? <LoaderCircle className="animate-spin" size={14} /> : <Trash2 size={14} />}Удалить</button>}
                      </div>
