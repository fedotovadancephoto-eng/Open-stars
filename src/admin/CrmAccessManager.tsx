...
import { Copy, KeyRound, LoaderCircle, ShieldCheck, UserPlus, X } from "lucide-react";
...
export function CrmAccessManager() {
  ...
  useEffect(() => onAdminSection("crm", () => {
    setCrmOpen(true);
    void fetchCrmContext().then((context) => setRole(context.role)).catch(() => setRole(null));
  }), []);
  ...
  const canManage = role === "owner" || role === "project_director";
  if (!crmOpen || !canManage) return null;
  ...
  return <>
    <button type="button" onClick={()=>void openPanel()} className="fixed bottom-5 right-5 z-[86] flex items-center gap-2 rounded-full border border-black/[0.06] bg-white px-4 py-3 text-sm font-semibold text-[#171717] shadow-[0_12px_35px_rgba(0,0,0,0.16)]">
      <ShieldCheck size={17} className="text-[#D96A24]"/> Доступы CRM
    </button>

    {panelOpen && <div className="fixed inset-0 z-[92] overflow-y-auto bg-black/30 p-3 backdrop-blur-sm sm:p-6" onClick={()=>setPanelOpen(false)}>
      <div className="mx-auto max-w-2xl rounded-[28px] bg-[#FAF9F5] p-5 shadow-2xl sm:p-7" onClick={e=>e.stopPropagation()}>
        ...
        <section className="mt-5 rounded-[22px] bg-white p-5">
          ...
          <button disabled={saving} onClick={()=>void createAccess()} className="mt-4 flex w-full items-center justify-center gap-2 rounded-[15px] bg-[#171717] py-3.5 text-sm font-semibold text-white disabled:opacity-50">{saving?<LoaderCircle size={16} className="animate-spin"/>:<KeyRound size={16}/>}Создать код на 7 дней</button>
