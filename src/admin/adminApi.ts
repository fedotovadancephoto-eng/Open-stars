...
export async function fetchAdminChildren(role: StaffRole): Promise<AdminChild[]> {
  ...
  const children = await restSelect<any>(
    "children",
    "select=id,family_id,first_name,last_name,group_name,branch,level,lesson_day,lesson_time,photo_url,coins,payment_status&order=last_name.asc,first_name.asc",
    token
  );
  ...
  return children.map((child) => {
    ...
    const invite = invites.find(
      (item) => item.family_id === child.family_id && !item.revoked_at
    );
