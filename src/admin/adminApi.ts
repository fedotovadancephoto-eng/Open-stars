...
export async function updateAdminChild(childId: string, input: ChildUpdateInput) {
  ...
  if (!firstName || !lastName) throw new Error("Имя и фамилия ребёнка обязательны.");
  ...
  await restPatch(
    ...
    `id=eq.${encodeURIComponent(childId)}`,
    {
      ...
      mentor_name: input.administratorName.trim() || null,
...
export async function fetchAdminChildren(role: StaffRole): Promise<AdminChild[]> {
  ...
  const basicChild = (child: any): Omit<AdminChild, "parentProfileId" | "parentName" | "parentPhone" | "activationStatus"> => ({
    id: child.id,
    ...
    administratorName: child.mentor_name || "",
  ...
  const [members, profiles, invites] = await Promise.all([
    ...
    restSelect<any>("parent_invites", "select=id,family_id,phone,full_name,claimed_at,revoked_at,expires_at&order=created_at.desc", token),
  ]);
  ...
  return children.map((child) => {
    ...
    const parentProfile: any = member ? profilesById.get(member.user_id) : null;
