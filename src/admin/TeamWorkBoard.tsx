...
import { GroupOccupancyBoard, GroupOccupancyTable } from "@/admin/GroupOccupancyBoard";
...
const metrics:Record<WorkMetric,string>={none:"Без числового показателя",calls:"Звонки",leads:"Заявки CRM",enrollment:"Действующие ученики",branch_report:"План и оплаты округа"};
...
function TaskEditor({context,task,busy,onAction,onClose}:{context:WorkContext;task?:WorkTask;busy:boolean;onAction:Action;onClose:()=>void}) {
 ...
 return <form className="mt-4 rounded-2xl border border-[#D96A24]/20 bg-orange-50 p-4" onSubmit={e=>{e.preventDefault();void onAction("save",{
  ...
  <fieldset disabled={busy} className="mt-3 grid gap-3 sm:grid-cols-2">
   ...
   <label className="text-xs">Показатель<select className={field} value={metric} onChange={e=>setMetric(e.target.value as WorkMetric)}>{Object.entries(metrics).filter(([k])=>k!=="branch_report").map(([k,v])=><option key={k} value={k}>{v}</option>)}</select></label>
...
function TaskCard({task,context,busy,onAction}:{task:WorkTask;context:WorkContext;busy:boolean;onAction:Action}) {
 ...
 const branchReport=task.metric==="branch_report";
 const [paidStudents,setPaidStudents]=useState(latest?.payload.paidStudents==null?"":String(latest.payload.paidStudents));
 const [dueStudents,setDueStudents]=useState(latest?.payload.dueStudents==null?"":String(latest.payload.dueStudents));
 const [paymentMonth,setPaymentMonth]=useState(latest?.payload.paymentMonth?.slice(0,7)||task.due_date.slice(0,7));
 ...
 return <article className="rounded-2xl border border-black/10 bg-white p-4">
  ...
  {!branchReport&&task.metric!=="none"&&<p className="mt-3 text-sm font-semibold">{metrics[task.metric]} · план: {task.planned_amount??"не задан"} · факт: {latest?.payload.actual??"нет отчёта"}</p>}
  {branchReport&&<div className="mt-3 rounded-xl bg-orange-50 p-3"><p className="font-semibold">Цель округа: {task.branchTarget??"не задана"} учеников</p><p className="mt-1 text-xs text-black/50">Администратор заполняет факт без выбывших и количество оплативших / ожидающих оплату за месяц. Это отчёт, он не создаёт платежи в ДДС.</p>{latest?.payload.paidStudents!=null&&<div className="mt-2 grid grid-cols-2 gap-2 text-sm"><p>Месяц: {latest.payload.paymentMonth?.slice(0,7)}</p><p>План на момент отчёта: {latest.payload.plan}</p><p>Факт: {latest.payload.actual}</p><p>Оплатили: {latest.payload.paidStudents}</p><p>Должны оплатить: {latest.payload.dueStudents}</p><p>Не хватает: {latest.payload.missingStudents}</p></div>}</div>}
  ...
  <div className="mt-3 flex flex-wrap gap-2">
   {!branchReport&&(mine||context.canReview)&&["todo","in_progress"].includes(task.status)&&<button disabled={busy} onClick={()=>setEdit(v=>!v)} className="rounded-xl bg-[#F7F5EF] px-3 py-2 text-sm">Изменить план</button>}
   ...
   {mine&&["todo","in_progress","returned"].includes(task.status)&&<button disabled={busy||task.due_date>context.today} onClick={()=>setReport(v=>!v)} className={button}>{branchReport?"Заполнить отчёт округа":"Отчитаться"}</button>}
  </div>
  {edit&&<TaskEditor context={context} task={task} busy={busy} onAction={onAction} onClose={()=>setEdit(false)}/>}
  {report&&<form className="mt-3 rounded-xl bg-[#F7F5EF] p-3" onSubmit={e=>{e.preventDefault();void act("report",{actual:actual===""?null:Number(actual),reached:reached===""?null:Number(reached),outcome,nextStep,paidStudents:paidStudents===""?null:Number(paidStudents),dueStudents:dueStudents===""?null:Number(dueStudents),paymentMonth:paymentMonth+"-01"});}}>
   <fieldset disabled={busy} className="grid gap-3 sm:grid-cols-2">
    {branchReport&&<><label className="text-xs">Месяц оплаты<input required type="month" className={field} value={paymentMonth} onChange={e=>setPaymentMonth(e.target.value)}/></label><label className="text-xs">Факт учеников · без выбывших<input required type="number" min="0" max="100000" className={field} value={actual} onChange={e=>setActual(e.target.value)}/></label><label className="text-xs">Сколько учеников оплатили<input required type="number" min="0" max={actual||"100000"} className={field} value={paidStudents} onChange={e=>setPaidStudents(e.target.value)}/></label><label className="text-xs">Сколько ещё должны оплатить<input required type="number" min="0" max={actual||"100000"} className={field} value={dueStudents} onChange={e=>setDueStudents(e.target.value)}/></label><p className="text-sm sm:col-span-2">Не хватает до цели: {actual===""||task.branchTarget==null?"—":Math.max(0,task.branchTarget-Number(actual))} учеников</p></>}
  ...
  {latest?.payload.groups&&<details className="mt-3"><summary className="cursor-pointer text-sm">Группы на момент отчёта</summary><GroupOccupancyTable rows={latest.payload.groups}/></details>}
  {task.events.length>0&&<details className="mt-3 text-xs text-black/50"><summary className="cursor-pointer">История задачи и отчётов ({task.events.length})</summary>{task.events.map(e=><div key={e.id} className="mt-2 border-t pt-2"><p>{new Date(e.created_at).toLocaleString("ru-RU",{timeZone:"Asia/Irkutsk"})} · {context.staff.find(s=>s.id===e.actor_id)?.name||"Проверяющий"} · {({plan:"Изменён план",start:"В работе",report:"Отчёт",accept:"Принято",return:"Возвращено",cancel:"Отменено"} as Record<string,string>)[e.action]||e.action}</p>{e.payload.outcome&&<p className="mt-1 whitespace-pre-wrap">Факт: {e.payload.actual??"—"} · {e.payload.outcome}</p>}{e.payload.paidStudents!=null&&<p className="mt-1">План: {e.payload.plan} · Оплатили: {e.payload.paidStudents} · Должны оплатить: {e.payload.dueStudents} · Не хватает: {e.payload.missingStudents} · Месяц: {e.payload.paymentMonth?.slice(0,7)}</p>}{e.payload.comment&&<p className="mt-1 whitespace-pre-wrap">{e.payload.comment}</p>}</div>)}</details>}
 </article>;
}
export function TeamWorkBoard() {
 ...
 return <div className="fixed inset-0 z-[95] overflow-y-auto bg-[#F7F5EF] p-4 sm:p-6"><div className="mx-auto max-w-6xl">
  ...
  {!context?<p className="py-10 text-black/50">{error?"План не загружен.":"Загружаем план команды…"}</p>:<>
   ...
   <GroupOccupancyBoard context={context} busy={busy} onAction={run}/>
