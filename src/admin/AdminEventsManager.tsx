...
import { CalendarDays, CheckCircle2, CirclePlus, CreditCard, LoaderCircle, RefreshCw, Search, WalletCards, X } from "lucide-react";
...
export function AdminEventsManager() {
  ...
  const visibleParticipants = useMemo(() => {
    ...
    return participants.filter((participant) => {
      ...
      return participant.status !== "declined" && (!text || [child?.fullName, child?.parentName, child?.parentPhone].join(" ").toLowerCase().includes(text));
    });
  ...
  async function savePayment() {
    ...
    if (!payParticipantId) return setError("Выберите участника.");
    if (!Number.isFinite(amount) || amount <= 0) return setError("Введите сумму оплаты.");
    setSaving(true); setError(""); setSuccess("");
    try {
      await confirmEventPayment({ participantId: payParticipantId, amount, paymentMethod: payMethod, receivedAt: new Date(`${payDate}T12:00:00`).toISOString(), note: payNote });
      setPayAmount(""); setPayNote(""); await loadEvent(selectedId); setSuccess("Оплата мероприятия сохранена и попала в ДДС мероприятий.");
  ...
  async function saveExpense() {
    ...
    try {
      ...
      setExpenseAmount(""); setExpenseDescription(""); await loadEvent(selectedId, true); setSuccess("Расход мероприятия сохранён.");
  ...
  return (
    <div className="fixed inset-0 z-[86] flex items-end justify-center bg-black/30 backdrop-blur-[2px] sm:items-center sm:p-5" onClick={() => !saving && setOpen(false)}>
      <div className="max-h-[96vh] w-full max-w-7xl overflow-y-auto rounded-t-[30px] bg-[#FAF9F5] p-5 shadow-2xl sm:rounded-[30px] sm:p-7" onClick={(event) => event.stopPropagation()}>
        ...
        <div className="mt-6 grid gap-5 lg:grid-cols-[0.72fr_1.28fr]">
          ...
          <section className="rounded-[24px] border border-black/[0.06] bg-white p-5 sm:p-6">
            {!selected ? <div className="grid min-h-[260px] place-items-center text-sm text-black/35">Выберите мероприятие.</div> : <>
              <div className="flex flex-wrap items-start justify-between gap-3"><div><p className="text-[11px] font-bold uppercase tracking-[0.16em] text-black/35">Карточка мероприятия</p><h3 className="mt-1 text-xl font-semibold">{selected.title}</h3><p className="mt-1 text-sm text-black/40">{dateTime(selected.startsAt)}{selected.location ? ` · ${selected.location}` : ""}</p></div>{isOwner && summary && <div className="grid grid-cols-3 gap-2 text-center"><div className="rounded-[14px] bg-[#5F6338]/[0.07] px-3 py-2"><p className="text-[10px] text-black/35">Доход</p><strong className="text-sm text-[#4D512E]">{money(summary.revenue)}</strong></div><div className="rounded-[14px] bg-[#D96A24]/[0.07] px-3 py-2"><p className="text-[10px] text-black/35">Расход</p><strong className="text-sm text-[#C95320]">{money(summary.expenses)}</strong></div><div className="rounded-[14px] bg-[#171717] px-3 py-2 text-white"><p className="text-[10px] text-white/45">Прибыль</p><strong className="text-sm">{money(summary.profit)}</strong></div></div>}</div>

              <div className="mt-5 flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between"><div><p className="text-[11px] font-bold uppercase tracking-[0.16em] text-[#5F6338]">Участники</p><p className="mt-1 text-sm text-black/45">Администратор фиксирует только факт оплаты.</p></div><div className="relative w-full sm:max-w-xs"><Search className="absolute left-3 top-1/2 h-4 w-4 -translate-y-1/2 text-black/30"/><input className="w-full rounded-[12px] border border-black/[0.07] bg-[#FAF9F5] py-2.5 pl-9 pr-3 text-sm outline-none" placeholder="Ребёнок или родитель" value={query} onChange={(e) => setQuery(e.target.value)}/></div></div>

              <div className="mt-3 max-h-[300px] space-y-2 overflow-y-auto">{visibleParticipants.length === 0 && <div className="rounded-[15px] bg-[#FAF9F5] p-6 text-center text-sm text-black/35">Пока никто не подтвердил участие.</div>}{visibleParticipants.map((participant) => { const child = childMap.get(participant.childId); const paid = paymentTotals.get(participant.id) || 0; return <button key={participant.id} type="button" onClick={() => { setPayParticipantId(participant.id); if (!payAmount) setPayAmount(String(participant.feeAmount ?? selected.defaultFee ?? "")); }} className={`w-full rounded-[15px] border p-3 text-left ${payParticipantId === participant.id ? "border-[#D96A24]/25 bg-[#D96A24]/[0.05]" : "border-black/[0.05] bg-[#FAF9F5]"}`}><div className="flex items-center justify-between gap-3"><div><p className="text-sm font-semibold">{child?.fullName || "Ребёнок"}</p><p className="mt-0.5 text-xs text-black/40">{child?.parentName || "Родитель"} · {child?.groupName || ""}</p></div><div className="text-right"><p className="text-xs font-semibold text-[#4D512E]">{paid > 0 ? `Оплачено ${money(paid)}` : "Не оплачено"}</p><p className="mt-0.5 text-[10px] text-black/35">Стоимость {participant.feeAmount == null ? "не задана" : money(participant.feeAmount)}</p></div></div></button>; })}</div>

              {payParticipantId && <div className="mt-4 rounded-[18px] border border-[#5F6338]/12 bg-[#5F6338]/[0.04] p-4"><div className="flex items-center gap-2"><CreditCard size={16} className="text-[#4D512E]"/><h4 className="text-sm font-semibold">Добавить оплату мероприятия</h4></div><div className="mt-3 grid gap-2 sm:grid-cols-4"><label className="text-[11px] font-semibold text-black/50">Сумма<input className={inputClass} inputMode="decimal" value={payAmount} onChange={(e) => setPayAmount(e.target.value)}/></label><label className="text-[11px] font-semibold text-black/50">Дата<input type="date" className={inputClass} value={payDate} onChange={(e) => setPayDate(e.target.value)}/></label><label className="text-[11px] font-semibold text-black/50">Способ<select className={inputClass} value={payMethod} onChange={(e) => setPayMethod(e.target.value as EventPaymentMethod)}>{paymentMethods.map(([value, label]) => <option key={value} value={value}>{label}</option>)}</select></label><label className="text-[11px] font-semibold text-black/50">Комментарий<input className={inputClass} value={payNote} onChange={(e) => setPayNote(e.target.value)}/></label></div><button type="button" disabled={saving} onClick={() => void savePayment()} className="mt-3 flex items-center gap-2 rounded-[12px] bg-[#5F6338] px-4 py-2.5 text-sm font-semibold text-white disabled:opacity-50"><CheckCircle2 size={16}/> Сохранить оплату</button></div>}

              {isOwner && <div className="mt-5 rounded-[18px] border border-black/[0.06] bg-[#FAF9F5] p-4"><div className="flex items-center gap-2"><WalletCards size={16} className="text-[#D96A24]"/><h4 className="text-sm font-semibold">Расходы мероприятия · только руководитель</h4></div><div className="mt-3 grid gap-2 sm:grid-cols-4"><label className="text-[11px] font-semibold text-black/50">Статья<select className={inputClass} value={expenseCategory} onChange={(e) => setExpenseCategory(e.target.value as EventExpenseCategory)}>{expenseCategories.map(([value, label]) => <option key={value} value={value}>{label}</option>)}</select></label><label className="text-[11px] font-semibold text-black/50">Сумма<input className={inputClass} inputMode="decimal" value={expenseAmount} onChange={(e) => setExpenseAmount(e.target.value)}/></label><label className="text-[11px] font-semibold text-black/50">Дата<input type="date" className={inputClass} value={expenseDate} onChange={(e) => setExpenseDate(e.target.value)}/></label><label className="text-[11px] font-semibold text-black/50">Комментарий<input className={inputClass} value={expenseDescription} onChange={(e) => setExpenseDescription(e.target.value)}/></label></div><button type="button" disabled={saving} onClick={() => void saveExpense()} className="mt-3 rounded-[12px] bg-[#171717] px-4 py-2.5 text-sm font-semibold text-white disabled:opacity-50">Добавить расход</button>{expenseRows.length > 0 && <div className="mt-4 divide-y divide-black/[0.06]">{expenseRows.slice(0, 8).map((row) => <div key={row.id} className="flex items-center justify-between gap-3 py-2 text-xs"><span className="text-black/45">{expenseCategories.find(([value]) => value === row.category)?.[1] || row.category} · {row.expenseDate}{row.description ? ` · ${row.description}` : ""}</span><strong>{money(row.amount)}</strong></div>)}</div>}</div>}
            </>}
          </section>
        </div>
        ...
        {success && <div className="mt-4 rounded-[14px] border border-[#5F6338]/15 bg-[#5F6338]/[0.07] px-4 py-3 text-sm text-[#4D512E]">{success}</div>}
