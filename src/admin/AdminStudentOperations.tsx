                    ...
                    <div className="flex items-start justify-between gap-3">
                      ...
                      {view === "active" ? (
                        <div className="flex shrink-0 flex-col gap-1.5">
                          <button type="button" onClick={() => { setArchiveChild(child); setReason(""); setOtherReason(""); setError(""); }} className="flex items-center justify-center gap-1.5 rounded-[11px] bg-[#171717] px-3 py-2 text-xs font-semibold text-white"><UserX size={14}/> Выбыл</button>
                          <button type="button" onClick={() => { setDeleteChild(child); setError(""); }} className="flex items-center justify-center gap-1.5 rounded-[11px] bg-red-50 px-3 py-2 text-[11px] font-semibold text-red-600"><Trash2 size={13}/> Не приступил</button>
                        </div>
                      ) : (
      ...
      {archiveChild && (
        <div className="fixed inset-0 z-[80] grid place-items-center bg-black/35 p-4 backdrop-blur-[2px]" onClick={() => setArchiveChild(null)}>
          <div className="w-full max-w-md rounded-[24px] bg-white p-5 shadow-2xl sm:p-6" onClick={(event) => event.stopPropagation()}>
            <div className="flex items-start justify-between gap-3"><div><p className="text-[11px] font-bold uppercase tracking-[0.18em] text-[#D96A24]">Выбыл</p><h3 className="mt-1 text-xl font-semibold">{archiveChild.fullName}</h3></div><button type="button" onClick={() => setArchiveChild(null)} className="grid h-9 w-9 place-items-center rounded-full bg-[#F2F0E8] text-black/50"><X size={18} /></button></div>
            <p className="mt-3 text-sm leading-6 text-black/45">Ребёнок исчезнет из активного списка и автоматически перейдёт в архив. Оценки, посещаемость, Star Coin, оплаты и вся история сохранятся.</p>
            ...
            <div className="mt-5 grid grid-cols-2 gap-2"><button type="button" onClick={() => setArchiveChild(null)} className="rounded-[13px] border border-black/[0.08] px-4 py-3 text-sm font-semibold text-black/55">Отмена</button><button type="button" disabled={busyId === archiveChild.id} onClick={confirmArchive} className="flex items-center justify-center gap-2 rounded-[13px] bg-[#171717] px-4 py-3 text-sm font-semibold text-white disabled:opacity-50">{busyId === archiveChild.id && <LoaderCircle className="animate-spin" size={16} />} Отметить «Выбыл»</button></div>
          </div>
        </div>
      )}

      {deleteChild && (
        <div className="fixed inset-0 z-[81] grid place-items-center bg-black/35 p-4 backdrop-blur-[2px]" onClick={() => setDeleteChild(null)}>
          <div className="w-full max-w-md rounded-[24px] bg-white p-5 shadow-2xl sm:p-6" onClick={(event) => event.stopPropagation()}>
            <div className="flex items-start justify-between gap-3"><div><p className="text-[11px] font-bold uppercase tracking-[0.18em] text-red-600">Удаление</p><h3 className="mt-1 text-xl font-semibold">{deleteChild.fullName}</h3></div><button type="button" onClick={() => setDeleteChild(null)} className="grid h-9 w-9 place-items-center rounded-full bg-[#F2F0E8] text-black/50"><X size={18} /></button></div>
            <p className="mt-3 text-sm leading-6 text-black/50">Это действие только для ребёнка, который <strong>вообще не приступил к занятиям</strong> и чей родитель не активировал кабинет. Система сама проверит учебную историю и не даст удалить действующего ученика.</p>
            <div className="mt-4 rounded-[14px] bg-red-50 px-4 py-3 text-xs leading-5 text-red-700">Если уже были оценки, посещаемость, Star Coin, оплаты или кабинет активирован — используйте «Выбыл», а не удаление.</div>
            {error && <div className="mt-4 rounded-[14px] border border-red-200 px-4 py-3 text-sm text-red-700">{error}</div>}
            <div className="mt-5 grid grid-cols-2 gap-2"><button type="button" onClick={() => setDeleteChild(null)} className="rounded-[13px] border border-black/[0.08] px-4 py-3 text-sm font-semibold text-black/55">Отмена</button><button type="button" disabled={busyId === deleteChild.id} onClick={confirmDelete} className="flex items-center justify-center gap-2 rounded-[13px] bg-red-600 px-4 py-3 text-sm font-semibold text-white disabled:opacity-50">{busyId === deleteChild.id ? <LoaderCircle className="animate-spin" size={16}/> : <Trash2 size={16}/>} Удалить</button></div>
          </div>
        </div>
      )}
    </>
