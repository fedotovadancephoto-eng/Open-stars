...
export function AdminStaffManager() {
  ...
  async function reactivate(person: StaffDirectoryRow) {
    if (busyId || saving || bulkBusy) return;
    if (!window.confirm(`Выдать ${person.fullName} новый код повторной активации? Карточка, роль, предметы и группы сохранятся. После активации прежний пароль перестанет подходить для входа.`)) return;
    setBusyId(person.profileId);
    setError("");
    setSuccess("");
    try {
      const next = await createStaffReactivationInvite(person);
      setReissuedCodes((current) => [...current.filter((code) => code.phone !== next.phone), next]);
      await refresh(actorRole);
      setSuccess(`Новый код для ${person.fullName} создан на 7 дней. Передайте сотруднику код и попросите выбрать «Активировать доступ».`);
    } catch (e) {
      setError(e instanceof Error ? e.message : "Не удалось создать код повторной активации.");
    } finally { setBusyId(""); }
  }
            ...
            <div className="space-y-5">
              <section className="rounded-[24px] border border-black/[0.06] bg-white p-5">
                ...
                {loading ? <div className="grid min-h-[140px] place-items-center"><LoaderCircle className="animate-spin text-black/25" /></div> : directory.length === 0 ? <p className="mt-4 text-sm text-black/40">Активных сотрудников пока нет.</p> : <div className="mt-4 max-h-[420px] space-y-2 overflow-y-auto">{directory.map(person => {
                  const accessCode = reissuedCodes.find((code) => code.phone === person.phone);
                  return <div key={person.profileId} className="rounded-[16px] border border-black/[0.055] p-3">
                    ...
                    {actorRole === 'owner' && person.roleName !== 'owner' && <div className="mt-3 flex flex-wrap gap-2"><button type="button" disabled={saving || bulkBusy || Boolean(busyId)} onClick={() => void reactivate(person)} className="flex items-center gap-1.5 rounded-[10px] bg-[#FFF3E9] px-3 py-2 text-xs font-semibold text-[#C95320] disabled:opacity-45">{busyId === person.profileId ? <LoaderCircle size={14} className="animate-spin" /> : <KeyRound size={14} />} Перевыпустить доступ</button><button type="button" disabled={saving || bulkBusy || Boolean(busyId)} onClick={() => void dismiss(person)} className="rounded-[10px] bg-red-50 px-3 py-2 text-xs font-semibold text-red-600 disabled:opacity-45">Уволить</button></div>}
                    {accessCode && <div className="mt-3 flex items-center justify-between gap-3 rounded-[12px] bg-[#FFF8F1] px-3 py-2.5"><div><p className="text-[10px] font-bold uppercase tracking-[0.14em] text-[#C95320]">Код повторной активации</p><p className="mt-1 font-mono text-2xl font-bold tracking-[0.16em]">{accessCode.activationCode}</p><p className="mt-1 text-[10px] text-black/35">Действует до {fmt(accessCode.expiresAt)}</p></div><button type="button" onClick={() => void copyReissuedCode(accessCode)} className="grid h-9 w-9 shrink-0 place-items-center rounded-lg bg-white text-black/45 shadow-sm" aria-label={`Скопировать код ${person.fullName}`}><Copy size={15} /></button></div>}
                  </div>;
                })}</div>}
