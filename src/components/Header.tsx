    ...
    <header className="sticky top-0 z-30 border-b border-neutral-200 bg-white/95 backdrop-blur-xl">
      <div className="mx-auto flex max-w-7xl items-center justify-between px-5 py-3.5 sm:px-6 lg:px-8">
        <div className="flex items-center gap-2.5">
          <button
            type="button"
            aria-label={panel === "menu" ? "Закрыть меню" : "Открыть меню"}
            aria-expanded={panel === "menu"}
            onClick={() => togglePanel("menu")}
            className="grid h-10 w-10 place-items-center rounded-full text-ink-soft transition-colors hover:bg-neutral-100 active:scale-95"
          >
            {panel === "menu" ? (
              <X className="h-5 w-5" />
            ) : (
        ...
        <div className="flex items-center gap-2">
          <button
            ...
            className="relative grid h-10 w-10 place-items-center rounded-full text-ink-soft transition-colors hover:bg-neutral-100 active:scale-95"
          >
          ...
          <button
            ...
            className="flex items-center gap-2 rounded-full border border-neutral-200 p-1 transition-colors hover:bg-neutral-50 active:scale-[0.98] sm:pr-3.5"
          >
            {child.photo ? (
              <img
                src={child.photo}
            ...
            {menuItems.map((item) => {
              ...
              return (
                <button
                  ...
                  className="flex min-h-[54px] items-center gap-2.5 rounded-[16px] px-3 text-left text-sm font-semibold text-[#171717] transition-colors hover:bg-[#FAF9F5] active:bg-[#F2F0E9]"
                >
                  <Icon className="h-5 w-5 shrink-0 text-[#D96A24]" strokeWidth={2} />
      ...
      {panel === "notifications" && (
        <div className="absolute right-4 top-[calc(100%+8px)] w-[min(360px,calc(100vw-32px))] rounded-[24px] border border-black/[0.06] bg-white p-5 shadow-[0_18px_50px_rgba(0,0,0,0.14)] sm:right-6">
          <div className="flex items-center gap-3">
            <div className="grid h-10 w-10 place-items-center rounded-full bg-[#D96A24]/10 text-[#C95320]">
              <Bell className="h-5 w-5" />
            </div>
            <div>
              <p className="font-semibold text-[#171717]">Уведомления</p>
              <p className="text-xs text-black/40">OPEN STARS</p>
            </div>
          </div>
          <div className="mt-4 rounded-[18px] bg-[#FAF9F5] px-4 py-5 text-center text-sm text-black/45">
        ...
        <div className="absolute right-4 top-[calc(100%+8px)] w-[min(340px,calc(100vw-32px))] rounded-[24px] border border-black/[0.06] bg-white p-5 shadow-[0_18px_50px_rgba(0,0,0,0.14)] sm:right-6">
          <div className="flex items-center gap-3">
            {child.photo ? (
              ...
              <div className="grid h-12 w-12 place-items-center rounded-full bg-[#F3F0E7] text-sm font-bold text-[#5F6338]">
                {initials}
              </div>
            )}
            <div className="min-w-0">
              ...
              <p className="mt-0.5 text-xs text-black/40">
                {child.groupName || child.group || "OPEN STARS"}
              </p>
            </div>
          </div>
          ...
          <button
            ...
            className="mt-4 flex w-full items-center justify-center gap-2 rounded-[16px] bg-[#171717] px-4 py-3 text-sm font-semibold text-white transition-opacity hover:opacity-90 active:scale-[0.99]"
          >
